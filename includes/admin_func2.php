 ...
 */
function atualizarEmpresa($conexao, $id, $dados) {
    $sql = "UPDATE empresas
            SET nome = ?, cnpj = ?, razao_social = ?, endereco = ?, contato = ?, telefone = ?, email = ?
            WHERE id = ?";  // Não atualiza o logo aqui.
    $stmt = $conexao->prepare($sql);
    try {
        $stmt->execute([
            $dados['nome'],
        ...
        // Trata erros de SQL, como CNPJ duplicado.
        if ($e->getCode() == 23000) { // Violação de constraint UNIQUE.
            preg_match("/Duplicate entry '(.*)' for key '(.*)'/", $e->getMessage(), $matches);
            if (!empty($matches)) {
                $valorDuplicado = $matches[1];
                $nomeCampo = $matches[2];
                $campo = ($nomeCampo == 'cnpj') ? 'CNPJ' : $nomeCampo; // Melhorar a mensagem.
                return "Erro: Já existe uma empresa com este $campo cadastrado.";
            }
        }
        error_log("Erro ao atualizar empresa: " . $e->getMessage()); // Log completo do erro.
        return "Erro inesperado ao atualizar a empresa.  Tente novamente."; // Mensagem genérica.
    }
}
 ...
 */
function excluirEmpresa($conexao, $id) {
 ...
 */
function getEmpresa($conexao, $id) {
