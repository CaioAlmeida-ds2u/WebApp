...
$planos_disponiveis = listarPlanosAssinatura($conexao, true);
$entidades_aplicaveis = [
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!validar_csrf_token($_POST['csrf_token'] ?? null)) {
        ...
        $dados_campo_form = [
            'nome_campo_interno' => trim($_POST['nome_campo_interno'] ?? ''),
            'label_campo_exibicao' => trim($_POST['label_campo_exibicao'] ?? ''),
            'tipo_campo' => $_POST['tipo_campo'] ?? '',
            'opcoes_lista_texto' => trim($_POST['opcoes_lista_texto'] ?? ''),
            'aplicavel_a_entidade' => $_POST['aplicavel_a_entidade'] ?? [],
            'disponivel_para_planos_ids' => $_POST['disponivel_para_planos_ids'] ?? [],
...
$lista_campos_pers = listarCamposPersonalizadosGlobaisPaginado($conexao);
error_log("Campos Personalizados Retornados: " . print_r($lista_campos_pers, true)); // Depuração
...
if ($edit_id_cp) {
    $campo_pers_para_editar = getCampoPersonalizadoGlobalPorId($conexao, $edit_id_cp);
    if (!$campo_pers_para_editar) {
        definir_flash_message('erro', "Campo para edição não encontrado (ID: $edit_id_cp).");
    } else {
                    ...
                    <tbody>
                        ...
                        <?php else: ?>
                            <?php foreach ($lista_campos_pers as $campo_p): ?>
                                <?php
                                // Validação para evitar Undefined array key
                                if (!isset($campo_p['id'], $campo_p['label_campo_exibicao'], $campo_p['nome_campo_interno'], $campo_p['tipo_campo'], $campo_p['ativo'])) {
                                    error_log("Campo personalizado inválido: " . print_r($campo_p, true));
                                ...
                                if (is_array($planos_ids_decoded)) {
                                    foreach ($planos_ids_decoded as $pid) {
                                        foreach ($planos_disponiveis as $pl) {
                                            if ($pl['id'] == $pid) {
                                                $planos_display_arr[] = $pl['nome_plano'];
                                                break;
                                            }
                                        }
                                    }
                                ...
                                <tr>
                                    ...
                                    <td class="text-center action-buttons-table">
                                        <button type="button" class="btn btn-sm btn-outline-primary me-1 action-btn btn-edit-campo-pers"
                                                ...
                                                data-action="editar"
                                                data-id="<?= htmlspecialchars($campo_p['id']) ?>"
                                                ...
                                                data-obrigatorio="<?= htmlspecialchars($campo_p['obrigatorio'] ?? 0) ?>"
                                                data-ativo="<?= htmlspecialchars($campo_p['ativo']) ?>"
                                        ...
                                        <form action="<?= $_SERVER['PHP_SELF'] ?>" method="POST" class="d-inline">
                                            ...
                                            <input type="hidden" name="action" value="excluir_campo_pers">
                                            <input type="hidden" name="campo_pers_id" value="<?= htmlspecialchars($campo_p['id']) ?>">
                                            <button type="submit" class="btn btn-sm btn-outline-danger" 
                                                    onclick="return confirm('Tem certeza que deseja excluir este campo personalizado?');">
                ...
                <div class="modal-body p-4">
                    ...
                    <div class="row g-3">
                        <div class="col-md-4">
                            ...
                            <input type="text" class="form-control form-control-sm" id="modal_label_campo_exibicao" name="label_campo_exibicao" 
                                   value="<?= htmlspecialchars($form_data_campo_pers['label_campo_exibicao'] ?? ($campo_pers_para_editar['label_campo_exibicao'] ?? '')) ?>" 
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="text" class="form-control form-control-sm" id="modal_nome_campo_interno" name="nome_campo_interno" 
                                   value="<?= htmlspecialchars($form_data_campo_pers['nome_campo_interno'] ?? ($campo_pers_para_editar['nome_campo_interno'] ?? '')) ?>" 
                                   required maxlength="50" pattern="[a-z0-9_]+" title="Apenas letras minúsculas, números e underscore.">
                        ...
                        <div class="col-md-4">
                            ...
                            <select class="form-select form-select-sm" id="modal_tipo_campo" name="tipo_campo" required>
                                ...
                                <?php foreach ($tipos_de_campo as $key => $label_tipo): ?>
                                    <option value="<?= $key ?>" <?= ($form_data_campo_pers['tipo_campo'] ?? ($campo_pers_para_editar['tipo_campo'] ?? '')) === $key ? 'selected' : '' ?>>
                        ...
                        <div class="col-12" id="container_opcoes_lista" style="display:none;">
                            ...
                            <textarea class="form-control form-control-sm" id="modal_opcoes_lista_texto" name="opcoes_lista_texto" rows="4" 
                                      placeholder="Uma opção por linha ou separadas por vírgula. Ex: Sim,Não,Talvez OU Pendente;Em Andamento;Concluído"><?= htmlspecialchars($form_data_campo_pers['opcoes_lista_texto'] ?? ($campo_pers_para_editar['opcoes_lista_texto'] ?? '')) ?></textarea>
                            ...
                            <div class="border rounded p-2 bg-light" style="max-height: 150px; overflow-y:auto;">
                                <?php foreach ($entidades_aplicaveis as $key_ent => $label_ent): ?>
                                    <div class="form-check form-check-inline">
                                        <input class="form-check-input" type="checkbox" name="aplicavel_a_entidade[]" value="<?= $key_ent ?>" 
                                               id="modal_ent_<?= $key_ent ?>" 
                                               <?= in_array($key_ent, (array)($form_data_campo_pers['aplicavel_a_entidade'] ?? ($campo_pers_para_editar['aplicavel_a_entidade'] ?? []))) ? 'checked' : '' ?>>
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="border rounded p-2 bg-light" style="max-height: 150px; overflow-y:auto;">
                                <?php if (empty($planos_disponiveis)): ?>
                                    <small class="text-muted">Nenhum plano cadastrado.</small>
                                <?php else: ?>
                                    <?php foreach ($planos_disponiveis as $plano_disp): ?>
                                        <div class="form-check">
                                            <input class="form-check-input" type="checkbox" name="disponivel_para_planos_ids[]" 
                                                   value="<?= $plano_disp['id'] ?>" id="modal_plano_<?= $plano_disp['id'] ?>" 
                                                   <?= in_array($plano_disp['id'], (array)($form_data_campo_pers['disponivel_para_planos_ids'] ?? ($campo_pers_para_editar['disponivel_para_planos_ids'] ?? []))) ? 'checked' : '' ?>>
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" class="form-control form-control-sm" id="modal_placeholder_campo" name="placeholder_campo" 
                                   value="<?= htmlspecialchars($form_data_campo_pers['placeholder_campo'] ?? ($campo_pers_para_editar['placeholder_campo'] ?? '')) ?>" maxlength="150">
                        </div>
                        <div class="col-md-6">
                            <label for="modal_texto_ajuda_campo" class="form-label form-label-sm">Texto de Ajuda (Abaixo do campo)</label>
                            <input type="text" class="form-control form-control-sm" id="modal_texto_ajuda_campo" name="texto_ajuda_campo" 
                                   value="<?= htmlspecialchars($form_data_campo_pers['texto_ajuda_campo'] ?? ($campo_pers_para_editar['texto_ajuda_campo'] ?? '')) ?>" maxlength="255">
                        </div>
                        ...
                        <div class="col-12 mt-3">
                            <div class="form-check form-switch mb-1">
                                <input class="form-check-input" type="checkbox" role="switch" id="modal_obrigatorio_campo_pers" name="obrigatorio_campo_pers" value="1" 
                                       <?= isset($form_data_campo_pers['obrigatorio_campo_pers']) || ($campo_pers_para_editar['obrigatorio'] ?? false) ? 'checked' : '' ?>>
                                <label class="form-check-label small" for="modal_obrigatorio_campo_pers">Obrigatório (usuário deve preencher)</label>
                            </div>
                            <div class="form-check form-switch">
                                <input class="form-check-input" type="checkbox" role="switch" id="modal_ativo_campo_pers" name="ativo_campo_pers" value="1" 
                                       <?= isset($form_data_campo_pers['ativo_campo_pers']) || ($campo_pers_para_editar['ativo'] ?? true) ? 'checked' : '' ?> checked>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    formsModalCampoPers.forEach(form => {
    ...
    if (modalCampoPers) {
        ...
        modalCampoPers.addEventListener('show.bs.modal', function(event) {
            ...
            if (action === 'editar' && button) {
                ...
                nomeInternoInput.readOnly = true;
